...
from BattleUtilities import dump_battle_data
...
class MinimaxPlayer(Player): 
    ...
    def choose_move(self, battle):
        ...
        # 20% of the time, do something random.  This is to inject new potential strategies
        if(self.useRandom and random.randint(1, 100) <= 20):
            action = self.choose_random_move(battle)
            dump_battle_data(battle, action)
            return action
        ...
        action = self.create_order(best_node.action)
        dump_battle_data(battle, action)
        return action
